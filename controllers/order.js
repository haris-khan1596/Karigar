...
async function getAllOrders(req, res) {
    const query = req.user.role === 'CUSTOMER' ? { customer: req.user._id, payment_status: { $ne: "PAID" } } : { worker: req.user._id, payment_status: { $ne: "PAID" }};
...
async function feedbackOrder(req,res) {
    ...
    await Promise.all([order.save(),firestore.collection("User").doc(`${order.worker}`).collection("orders").doc(`${order._id}`).update({"rating": req.body.rating,"feedback": req.body.feedback}),
    firestore.collection("User").doc(`${order.customer}`).collection("orders").doc(`${order._id}`).update({"rating": req.body.rating,"feedback": req.body.feedback})]);
...
async function cancelOrder(req,res) {
    ...
    await Promise.all([order.save(),firestore.collection("User").doc(`${order.customer}`).collection("orders").doc(`${order._id}`).update({"status": "CANCELLED","reason": req.body.reason}),
    firestore.collection("User").doc(`${order.worker}`).collection("orders").doc(`${order._id}`).update({"status": "CANCELLED","reason": req.body.reason})]);
...
async function completeOrder(req,res) {
    ...
    await Promise.all([order.save(),firestore.collection("User").doc(`${order.worker}`).collection("orders").doc(`${order._id}`).update({"status": "COMPLETED","price": req.body.price}),firestore.collection("User").doc(`${order.customer}`).collection("orders").doc(`${order._id}`).update({"status": "COMPLETED","price": req.body.price})]);
...
async function startedOrder(req,res) {
    ...
    await Promise.all([order.save(),firestore.collection("User").doc(`${order.worker}`).collection("orders").doc(`${order._id}`).update({"status": "STARTED"}),firestore.collection("User").doc(`${order.customer}`).collection("orders").doc(`${order._id}`).update({"status": "STARTED"})]);
...
async function sentPayment(req,res){
    ...
    await Promise.all([order.save(),firestore.collection("User").doc(`${order.worker}`).collection("orders").doc(`${order._id}`).update({"payment_status": "SENT"}),firestore.collection("User").doc(`${order.customer}`).collection("orders").doc(`${order._id}`).update({"payment_status": "SENT"})]);
...
async function paidPayment(req,res){
    ...
    await Promise.all([order.save(),firestore.collection("User").doc(`${order.worker}`).collection("orders").doc(`${order._id}`).update({"payment_status": "PAID"}),firestore.collection("User").doc(`${order.customer}`).collection("orders").doc(`${order._id}`).update({"payment_status": "PAID"})]);
...
async function unpaidPayment(req,res){
    ...
    await Promise.all([order.save(),firestore.collection("User").doc(`${order.customer}`).collection("orders").doc(`${order._id}`).update({"payment_status": "UNPAID"}),firestore.collection("User").doc(`${order.worker}`).collection("orders").doc(`${order._id}`).update({"payment_status": "UNPAID"})]);
